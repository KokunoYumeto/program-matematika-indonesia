...
assert.match(effectiveCoursesById.get('B95').zenodo, /22261912$/);
...
assert.match(livePublicationsModule, /22261912/);
...
assert.equal(b30Reader.standalone_reader_pages, 68);
assert.equal(b30Reader.embedded_fragment_paths.length, 356);
assert.equal(b30Reader.fragment_url_base_marker, 'b30-fragment-url-base');
const b30EnglishAdmission = await readJson('docs/en/courses/B30/B30_ORIGINAL_ENGLISH_ADMISSION_V1.json');
assert.equal(b30EnglishAdmission.translation_performed, false);
assert.deepEqual(b30EnglishAdmission.coverage, {chapter:3, section:48, exercise:737, hint:721, answer:737, solution:737, proof:31, theorem:46, definition:35, example:253});
assert.equal(b30EnglishAdmission.source_revision, 'd84ac1a86f71a7cac4946c53df9ed19e32a8c28e');
assert.equal(b30EnglishAdmission.source_unit_routes, 53501);
const b20Reader = centralNavigation.readers.find(row => row.course_id === 'B20' && row.locale === 'en');
assert.equal(b20Reader.root, 'docs/en/courses/B20/reader');
assert.equal(b20Reader.html_documents, 406);
assert.equal(b20Reader.standalone_reader_pages, 83);
assert.equal(b20Reader.embedded_fragment_paths.length, 323);
const b20EnglishAdmission = await readJson('docs/en/courses/B20/B20_ORIGINAL_ENGLISH_ADMISSION_V1.json');
assert.equal(b20EnglishAdmission.translation_performed, false);
assert.deepEqual(b20EnglishAdmission.coverage, {chapter:5, section:62, exercise:695, hint:620, answer:695, solution:695, proof:15, theorem:41, definition:43, example:208});
assert.equal(b20EnglishAdmission.source_revision, '9f0295936d395bec68dab7915057135a2c7f0414');
assert.equal(b20EnglishAdmission.source_unit_routes, 45821);
