...
const publishedCount = capsules.filter(({ course }) => course.state === 'published').length;
const productionIds = capsules.filter(({ course }) => course.state === 'production').map(({ course_id }) => course_id);
...
// The merged authority carries explicit anonymous publication evidence for A30.
// Promote the native-course status only when that evidence is present; otherwise
// retain the conservative published-course fallback used by the historical line.
assert.equal(a30.course_native.status, overrides.course_truth?.A30?.publication_evidence ? 'verified' : 'available_unverified');
...
assert.equal(c60.layers.interoperability.semantic_adapter.evidence[0].sha256, 'f0f9eae897f65c9c3815de5bc8200b1896dc6adbe44e94993b39291b460501d3');
assert.equal(c60.layers.interoperability.semantic_adapter.evidence[1].sha256, 'f6e68b03f77adb2e7c9b51eca9adb1a5d7a0e55a1d0a90befc00ee48e5c83273');
...
assert.deepEqual(c60.layers.learner.tools[0].page, overrides.learner_tools.C60[0].page);
...
assert.ok(c60.layers.educator.resources.some(resource => resource.id === 'C60:educator-hub-v1' && resource.status === 'verified'));
assert.ok(c60.layers.educator.resources.some(resource => resource.id === 'C60:native-id-index-v1' && resource.status === 'verified'));
...
assert.equal(d90.layers.interoperability.semantic_adapter.evidence[0].sha256, '9df3955d918c1c010be7e7b8beca01dcc199b9edfe7686966ac049dc4585663f');
assert.equal(d90.layers.interoperability.semantic_adapter.evidence[1].sha256, '74d93103d66bddd11c1c12dadceb0209d004547c29685017b81aff5b603ddfe7');
...
assert.deepEqual(d90.layers.learner.tools[0].page, overrides.learner_tools.D90[0].page);
...
assert.ok(d90.layers.educator.resources.some(resource => resource.id === 'D90:educator-hub-v1' && resource.status === 'verified'));
assert.ok(d90.layers.educator.resources.some(resource => resource.id === 'D90:educator-map-v1' && resource.status === 'verified'));
