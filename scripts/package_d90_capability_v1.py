"""Create the deterministic metadata-only D90 capability packet."""
...
from validate_d90_capability_v1 import validate


ADAPTER_REL = Path("backend/course-capsule-v1/adapters/d90-capability-v1")
DEFAULT_OUTPUT = DEFAULT_ADAPTER / "build"
ARCHIVE_NAME = "D90_THIN_CAPABILITY_METADATA_V1.zip"
RECEIPT_NAME = "PACKET_BUILD_RECEIPT.json"
SCRIPT_NAMES = (
    "d90_capability_model_v1.py",
    "build_d90_capability_v1.py",
    "validate_d90_capability_v1.py",
    "package_d90_capability_v1.py",
)
FORBIDDEN_SUFFIXES = {
    ".pdf",
    ".tex",
...
def safe_relative_path(value: str) -> PurePosixPath:
    ...
    if (
        ...
        raise PacketError("D90-PACKET-UNSAFE-PATH:" + value)
    return path
...
def _add_member(members: dict[str, bytes], name: str, path: Path) -> None:
    ...
    if not safe_member(normalized) or normalized in members:
        raise PacketError("D90-PACKET-MEMBER-PATH:" + normalized)
    if path.suffix.casefold() in FORBIDDEN_SUFFIXES:
        raise PacketError("D90-PACKET-FORBIDDEN-PAYLOAD:" + normalized)
    data = path.read_bytes()
    if LOCAL_PROFILE_PATTERN.search(data):
        raise PacketError("D90-PACKET-LOCAL-PROFILE-DATA:" + normalized)
...
def collect_members(adapter: Path) -> dict[str, bytes]:
    ...
    for path in required:
        if not path.is_file():
            raise PacketError("D90-PACKET-REQUIRED-FILE-MISSING:" + path.name)

    manifest = read_json(adapter / "manifest.json")
    declared = {
        safe_relative_path(str(row["path"])) for row in manifest.get("outputs", [])
    }
    ...
    if actual_adapter != allowed_adapter:
        ...
        raise PacketError(
            f"D90-PACKET-ADAPTER-FILE-SET:missing={missing}:extra={extra}"
        )
    ...
    start_here = (
        ...
        "This packet contains only the D90 zero-copy adapter metadata, generated views, "
        "validation evidence, negative fixtures, and four standard-library Python scripts. "
        "It does not contain the native JSONL/CSV dataset, course prose, solution bodies, "
        "TeX, HTML, PDF, or EPUB.\n\n"
        "Deterministic rebuild and validation require a separate checkout of "
        "https://github.com/KokunoYumeto/advanced-optimization-convex-analysis-id at commit "
        f"`{RELEASE_COMMIT}` (tree `{RELEASE_TREE}`). Then run:\n\n"
        ...
        "The adapter preserves identity/state/evidence and stable public anchors; it does not "
        "claim reversible exchange with the native representation.\n"
    ).encode("utf-8")
    members["START_HERE.md"] = start_here
    inventory = {
        "schema": "d90-capability-thin-packet-inventory/1",
        "course_id": COURSE_ID,
        ...
        "excluded_payload_classes": [
            ...
            "course prose and solution bodies",
            "TeX, HTML, PDF, and EPUB native artifacts",
...
def verify_zip(payload: bytes, members: dict[str, bytes]) -> dict[str, Any]:
    with zipfile.ZipFile(io.BytesIO(payload), "r") as archive:
        ...
        if names != sorted(members) or len(names) != len(set(names)):
            raise PacketError("D90-PACKET-ZIP-INVENTORY")
        if archive.testzip() is not None:
            raise PacketError("D90-PACKET-ZIP-CRC")
        for info in infos:
            if not safe_member(info.filename):
                raise PacketError("D90-PACKET-ZIP-PATH:" + info.filename)
            if info.date_time != (1980, 1, 1, 0, 0, 0) or info.extra or info.comment:
                raise PacketError("D90-PACKET-ZIP-METADATA:" + info.filename)
            data = archive.read(info.filename)
            if data != members[info.filename] or info.file_size != len(data):
                raise PacketError("D90-PACKET-ZIP-BYTES:" + info.filename)
            suffix = PurePosixPath(info.filename).suffix.casefold()
            if suffix in FORBIDDEN_SUFFIXES:
                raise PacketError("D90-PACKET-ZIP-FORBIDDEN-PAYLOAD:" + info.filename)
            if LOCAL_PROFILE_PATTERN.search(data):
                raise PacketError("D90-PACKET-ZIP-LOCAL-PROFILE:" + info.filename)
    return {
...
def package(native: Path, adapter: Path, output: Path) -> dict[str, Any]:
    ...
    if first != second:
        raise PacketError("D90-PACKET-TWO-BUILD-DRIFT")
    ...
    receipt = {
        "schema": "d90-capability-thin-packet-build-receipt/1",
        "course_id": COURSE_ID,
        "result": "PASS",
        "archive": {
            **identity(
                archive_path,
                display_path=(Path("build") / ARCHIVE_NAME).as_posix(),
            ),
        },
...
def main() -> int:
    ...
    receipt = package(
        args.native_root.resolve(), args.adapter.resolve(), args.output.resolve()
    )
    print(
        json.dumps(
            {
                ...
                "members": receipt["zip_checks"]["member_count"],
            },
            sort_keys=True,
        )
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"D90 packet build failed: {error}", file=sys.stderr)
        sys.exit(1)
