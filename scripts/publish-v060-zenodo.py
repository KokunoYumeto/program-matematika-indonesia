#!/usr/bin/env python3
"""Publish PMI v0.60.0 to its reserved Zenodo version and verify both remotes."""
...
RECORD_ID = 22143506
CONCEPT_ID = 22059707
PREDECESSOR_ID = 22133203
AUTHORITY_SOURCE_COMMIT = "328d2a41b544db477d869d2821a640f0a947f153"
RELEASE_COMMIT = "26b7e9bde648918e65170ea881e709c5dfa8fe43"
...
LOGBOOK_RECEIPT = LOGBOOK / "125_CENTRAL_V060_PUBLICATION_RECEIPT_20260828.json"
...
TITLE = "Program Matematika Indonesia v0.60.0 — Mulai Belajar dan Peta Kurikulum Terbuka"
DESCRIPTION = (
    ...
    "<p>Rilis v0.60.0 mempertahankan peta kurikulum matematika terbuka dan menambahkan inventaris asesmen owner-native O001/A00 "
    "yang tervalidasi, kontrak kapabilitas global, serta paket backend v2.2. Program keseluruhan belum lengkap; edisi yang masih "
    "diproduksi tetap ditandai secara eksplisit. Untuk mulai tanpa browser, buka "
    "<code>00_MULAI_BELAJAR_PROGRAM_MATEMATIKA_INDONESIA_v0.60.0.pdf</code> atau "
    "<code>01_MULAI_BELAJAR_PROGRAM_MATEMATIKA_INDONESIA_v0.60.0.html</code>.</p>"
    "<p>Lapisan mesin tersedia sebagai pendamping sekunder: katalog kurikulum, authority snapshot, backend federasi v2 dan v2.1, "
    "backend modular v2.2, kontrak kapabilitas global, dan shard asesmen O001/A00. Setiap korpus tetap mempertahankan atribusi, "
...
NOTES = (
    "Snapshot terverifikasi v0.60.0. Program keseluruhan belum lengkap; status lengkap dan produksi berjalan ditampilkan secara "
    "eksplisit. Bahan tambahan dengan lisensi berbeda dipisahkan dari kursus utama."
)
...
def local_inventory() -> list[dict[str, object]]:
    ...
    require(len(names) == 57, f"release directory must contain 57 files, found {len(names)}")
    ...
    require(len(checksums) == 56, "CHECKSUMS.sha256 must bind 56 payload files")
    ...
    require(set(declared) == expected, "checksum inventory does not bind exactly the other 56 files")
    for row in rows:
...
def catalog_and_related_identifiers() -> tuple[dict[str, object], list[dict[str, str]]]:
    ...
    require(isinstance(dois, list) and len(dois) == 18, "catalog must bind 18 completed public record DOIs")
    normalized: list[str] = []
    for value in dois:
...
def publish_zenodo(
    ...
    if public_existing.status_code == 200:
        ...
        inventory = verify_public_zenodo(record, local, creators, contributors)
        creator_hash = sha256_bytes(json.dumps(creators, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8"))
        contributor_hash = sha256_bytes(json.dumps(contributors, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8"))
    ...
    if concept_id is not None:
        require(int(concept_id) == CONCEPT_ID, "Zenodo draft concept differs")
    ...
    require(latest.json().get("id") == RECORD_ID, "Zenodo concept latest does not resolve to v0.60.0")
    resolver = requests.get(f"https://doi.org/10.5281/zenodo.{RECORD_ID}", timeout=60)
    resolver.raise_for_status()
    require(str(RECORD_ID) in resolver.url, "DOI resolver does not end at the published record")
    return record, inventory, creator_hash, contributor_hash
...
def write_receipt(
    ...
    receipt = {
        "schema_id": "program-matematika-indonesia/combined-publication-receipt/v16",
        "version": VERSION,
        ...
        "github": {
            ...
            "anonymous_filename_size_sha256_readback": "pass_57_of_57",
        },
        "zenodo": {
            "record_id": RECORD_ID,
        ...
        "backend_state": {
            ...
            "qa_events": 17,
            "assessment_items": 8105,
            "assessment_solutions": 5240,
            "assessment_solution_gaps": 2865,
        },
