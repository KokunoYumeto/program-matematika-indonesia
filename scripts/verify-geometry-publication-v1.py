"""Resumable anonymous readback of the bounded C100 Geometry integration."""
...
scope = [
    ...
    'releases/geometry-learning-capability-v1',
]
paths = subprocess.check_output([
    'git', 'diff', '--name-only', '--diff-filter=AM', args.base_commit, args.commit, '--', *scope,
...
target = ROOT / 'backend/course-capsule-v1/adapters/geometry-capability-v1/publication' / (
    'GITHUB_READBACK_' + args.commit[:12] + '.json'
)
target.parent.mkdir(parents=True, exist_ok=True)
receipt = json.loads(target.read_bytes()) if target.exists() else {
    'schema': 'geometry-integration-public-readback/1',
    'state': 'in_progress',
    ...
    'files': [],
    'failures': [],
    'scope': 'Changed C100 integration source files at the exact commit and every changed docs file on GitHub Pages.',
}
assert receipt['source_commit'] == args.commit and receipt['base_commit'] == args.base_commit
session = requests.Session()
session.trust_env = False
session.headers['User-Agent'] = 'geometry-capability-anonymous-readback'
...
receipt['failures'] = []
jobs = [
    ('source', path, 'https://raw.githubusercontent.com/' + repo + '/' + args.commit + '/' + quote(path))
    for path in paths
]
jobs += [
    ('pages', path, 'https://kokunoyumeto.github.io/program-matematika-indonesia/' + quote(path[5:]))
    for path in paths if path.startswith('docs/')
]
for surface, path, url in jobs:
    if (surface, path) in complete:
        prior = next(row for row in receipt['files'] if (row['surface'], row['path']) == (surface, path))
