"""Resumable anonymous readback of the bounded D40 integration."""
...
target = ROOT / "backend/course-capsule-v1/adapters/d40-capability-v1/publication" / (
    "GITHUB_READBACK_" + args.commit[:12] + ".json"
)
target.parent.mkdir(parents=True, exist_ok=True)
receipt = json.loads(target.read_bytes()) if target.exists() else {
    "schema": "d40-integration-public-readback/1",
    "state": "in_progress",
    ...
    "failures": [],
    "scope": (
        "Changed D40 adapter, integration, package and generated learner-site files "
        "at the exact commit, plus every changed docs file on GitHub Pages."
    ),
}
assert receipt["source_commit"] == args.commit and receipt["base_commit"] == args.base_commit
session = requests.Session()
session.trust_env = False
session.headers["User-Agent"] = "d40-capability-anonymous-readback"
