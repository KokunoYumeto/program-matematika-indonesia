"""Resumable anonymous readback of the bounded C90 topology integration."""
...
scope = [
    ...
    "releases/topology-learning-capability-v1",
    "pnpm-workspace.yaml",
]
...
target = ROOT / "backend/course-capsule-v1/adapters/topology-capability-v1/publication" / (
    "GITHUB_READBACK_" + args.commit[:12] + ".json"
)
target.parent.mkdir(parents=True, exist_ok=True)
receipt = json.loads(target.read_bytes()) if target.exists() else {
    "schema": "topology-integration-public-readback/1",
    "state": "in_progress",
    ...
    "failures": [],
    "scope": (
        "Changed C90 integration and packet files at the exact commit, plus every "
        "changed docs file on GitHub Pages."
    ),
}
assert receipt["source_commit"] == args.commit and receipt["base_commit"] == args.base_commit
session = requests.Session()
session.trust_env = False
session.headers["User-Agent"] = "topology-capability-anonymous-readback"
