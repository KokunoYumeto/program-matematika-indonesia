#!/usr/bin/env python3
"""Publish and anonymously verify the bounded D110 v2.3.1 source admission."""
...
TOKEN_FILE_ENV = "PMI_D110_GITHUB_TOKEN_FILE"
RECEIPT = ROOT / "GITHUB_D110_V231_SOURCE_PUBLICATION_RECEIPT.json"
OWNER = "KokunoYumeto"
REPO_NAME = "program-matematika-indonesia"
REPO = f"{OWNER}/{REPO_NAME}"
COMMIT_MESSAGE = "Admit the D110 Mathematics in Lean backend v2.3.1 adapter"
...
def load_base():
    ...
    module.USER_AGENT = "Codex-PMI-D110-v231-Source-Publisher/1.0"
    return module
...
def source_files() -> dict[str, Path]:
    names = {
        ...
        "backend/v2.3/scripts/build_d110_v23_adapter.py": ROOT / "backend/v2.3/scripts/build_d110_v23_adapter.py",
        "backend/v2.3/scripts/validate_d110_v23_adapter.py": ROOT / "backend/v2.3/scripts/validate_d110_v23_adapter.py",
        ...
        "scripts/publish-d110-v231-source.py": Path(__file__).resolve(),
    ...
    extension = ROOT / "backend/v2.3/extensions/d110-mathematics-in-lean-v0.1.0"
    extension_files = sorted(item for item in extension.rglob("*") if item.is_file())
    if len(extension_files) != 59:
        raise RuntimeError("canonical D110 extension file count is not 59")
    ...
    result = dict(sorted(names.items()))
    if len(result) != 71:
        raise RuntimeError("bounded D110 source snapshot is not exactly 71 files")
    return result
...
def main() -> None:
    ...
    if live_ref["object"]["sha"] != commit_sha:
        raise RuntimeError("main ref did not retain the D110 source commit")
    workflow = base.wait_for_pages(token, commit_sha, changed)
    raw_checks, page_checks = anonymous_readback(base, commit_sha, snapshot)
    receipt = {
        "schema_id": "program-matematika-indonesia/github-d110-v231-source-publication/1.0.0",
        ...
        "learner_primary_url": "https://kokunoyumeto.github.io/mathematics-in-lean-id/",
