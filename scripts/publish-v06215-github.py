#!/usr/bin/env python3
"""Publish or anonymously verify the exact 112-asset PMI v0.62.15 GitHub release.
...
boundary affected by v0.62.15.  The default mode remains anonymous verify-only;
...
v0.62.15 build has completed.  No dry-run, verification, or publication may
...
RELEASE_DIR = PROJECT / "releases/v0.62.15"
RECEIPT_PATH = PROJECT / "GITHUB_PUBLICATION_RECEIPT_v0.62.15.json"
PREDECESSOR_RECEIPT = PROJECT / "GITHUB_PUBLICATION_RECEIPT_v0.62.14.json"
PREDECESSOR_ZENODO_RECEIPT = PROJECT / "PUBLICATION_RECEIPT_v0.62.14.json"
CHECKSUM_NAME = "RELEASE_CHECKSUMS_v0.62.15.sha256"
SOURCE_ARCHIVE_NAME = "program-matematika-indonesia-source-v0.62.15.zip"
...
EXPECTED_PREDECESSOR_ZENODO_RECEIPT = (
    58_227,
...
EXPECTED_PREDECESSOR_ZENODO_RECORD_ID = 22_217_240
...
UNRESOLVED = "__UNRESOLVED_V0_62_15__"
CONFIGURATION_FINALIZED = True
EXPECTED_SUCCESSOR_TOTAL_BYTES: int | None = 745_035_211
EXPECTED_SUCCESSOR_AGGREGATE_SHA256 = "62741d6fe384cd73b059c9d267b5703a9511f40ab6557bcfa5bd1aeaff808d5f"
EXPECTED_CHECKSUM_IDENTITY: tuple[int | None, str] = (
    12_461,
    "801daa24e45ee83fbc5c71088dc6dec8685d0e4551ceecd418fa5d342d0d6178",
)
EXPECTED_SOURCE_ARCHIVE_IDENTITY: tuple[int | None, str] = (
    508_950_443,
    "dee2c0b5149d8049c3568002b164ae9e1ecad50aa8adbf46ebc3e17bfd842c77",
)
...
RELEASE_NAME = "Program Matematika Indonesia v0.62.15 — backend modular terpadu"
...
Rilis {TAG} mempertahankan 96 aset v0.62.14 secara byte-identik, mengganti tiga belas
aset backend, dan mengganti tiga berkas rilis berversi. Sembilan peran kurikulum
terikat ke delapan paket adapter yang seluruhnya telah diterbitkan dan dibaca balik
secara publik. Sebanyak 31 peran tetap tanpa klaim adapter lokal. Paket kapsul juga
memuat kebijakan terminologi berbasis
...
_overrides = {
    ...
    "USER_AGENT": "Codex-PMI-v06215-GitHub-Publisher/1.0",
...
def validate_configuration() -> None:
    ...
    require(EXPECTED_PREDECESSOR_FILES == 112 and EXPECTED_FILES == 112, "predecessor/successor counts differ")
...
def predecessor_inventory() -> dict[str, dict[str, Any]]:
    require(PREDECESSOR_RECEIPT.is_file() and not PREDECESSOR_RECEIPT.is_symlink(), "v0.62.14 GitHub receipt is missing")
    data = PREDECESSOR_RECEIPT.read_bytes()
    require((len(data), sha256_bytes(data)) == EXPECTED_PREDECESSOR_RECEIPT, "v0.62.14 GitHub receipt identity differs")
    try:
        receipt = json.loads(data.decode("utf-8"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise VerificationError("v0.62.14 GitHub receipt is not valid UTF-8 JSON") from exc
    require(isinstance(receipt, dict), "v0.62.14 GitHub receipt is not an object")
    require(receipt.get("version") == PREDECESSOR_VERSION, "predecessor version differs")
    require(receipt.get("state") == "published_public_verified", "predecessor GitHub state differs")
    ...
    require(
        ...
        "predecessor GitHub inventory authority differs",
    )
    require(
        isinstance(release, dict)
        and release.get("id") == EXPECTED_PREDECESSOR_RELEASE_ID
        and release.get("tag") == f"v{PREDECESSOR_VERSION}"
        ...
        "predecessor GitHub release authority differs",
    )
    require(
        isinstance(readback, dict)
        and readback.get("result") == "pass_112_of_112",
        "predecessor GitHub anonymous readback differs",
    )
    payload = readback.get("entries")
    require(isinstance(payload, list) and len(payload) == EXPECTED_PREDECESSOR_FILES, "predecessor GitHub inventory is not 112 rows")
    ...
    for raw in payload:
        ...
        require(
            isinstance(url, str)
            and url == f"https://github.com/{REPOSITORY_SLUG}/releases/download/v{PREDECESSOR_VERSION}/{name}",
    ...
    require(
        PREDECESSOR_ZENODO_RECEIPT.is_file() and not PREDECESSOR_ZENODO_RECEIPT.is_symlink(),
        "v0.62.14 Zenodo receipt is missing",
    )
    zenodo_data = PREDECESSOR_ZENODO_RECEIPT.read_bytes()
    require(
        (len(zenodo_data), sha256_bytes(zenodo_data)) == EXPECTED_PREDECESSOR_ZENODO_RECEIPT,
        "v0.62.14 Zenodo receipt identity differs",
    )
    try:
        zenodo_receipt = json.loads(zenodo_data.decode("utf-8"))
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise VerificationError("v0.62.14 Zenodo receipt is not valid UTF-8 JSON") from exc
    require(isinstance(zenodo_receipt, dict), "v0.62.14 Zenodo receipt is not an object")
    zenodo = zenodo_receipt.get("zenodo")
    github_authority = zenodo_receipt.get("github_authority")
    require(
        zenodo_receipt.get("version") == PREDECESSOR_VERSION
        and zenodo_receipt.get("state") == "published_open_modular_backend_successor"
        and zenodo_receipt.get("payload_total_bytes") == EXPECTED_PREDECESSOR_ZENODO_TOTAL_BYTES
        and zenodo_receipt.get("payload_inventory_aggregate_sha256") == EXPECTED_PREDECESSOR_ZENODO_AGGREGATE,
        "predecessor Zenodo receipt authority differs",
    )
    require(
        isinstance(zenodo, dict)
        and zenodo.get("record_id") == EXPECTED_PREDECESSOR_ZENODO_RECORD_ID
        ...
        "predecessor Zenodo public authority differs",
    )
    require(
        isinstance(github_authority, dict)
        and github_authority.get("anonymous_readback") == "pass_112_of_112"
        and github_authority.get("tag_target_commit") == EXPECTED_PREDECESSOR_COMMIT
        and github_authority.get("source_tree") == EXPECTED_PREDECESSOR_TREE,
        "predecessor Zenodo receipt GitHub binding differs",
    )
    return by_name
...
def validate_release_boundary(local_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    require(pure_omissions == PURE_OMISSIONS, "v0.62.15 pure-omission set differs")
    require(pure_additions == PURE_ADDITIONS, "v0.62.15 pure-addition set differs")
    require(changed == SAME_NAME_REPLACEMENTS, "v0.62.15 replacement set differs")
    require(len(unchanged) == EXPECTED_UNCHANGED, "v0.62.15 unchanged count differs")
    ...
    return {
        ...
        "result": "pass_exact_96_unchanged_13_replacements_3_omissions_3_pure_new",
    }
...
def preflight() -> dict[str, Any]:
    validate_configuration()
    require(RELEASE_DIR == PROJECT / "releases/v0.62.15", "release directory boundary differs")
    require(RECEIPT_PATH == PROJECT / "GITHUB_PUBLICATION_RECEIPT_v0.62.15.json", "receipt path differs")
