#!/usr/bin/env python3
"""Preflight or assemble the exact 112-file PMI v0.62.15 release.
...
* retain 96 files byte-for-byte from the verified public v0.62.14 GitHub payload;
* replace thirteen predecessor filenames with commit-bound v0.62.15 bytes;
* omit only the three superseded v0.62.14 versioned release files;
* add the three v0.62.15 versioned release files;
...
is anonymous predecessor hydration, which is bounded by the pinned v0.62.14
GitHub receipt and refuses to overwrite an existing predecessor path.
"""
...
PREDECESSOR_RECEIPT = PROJECT / "PUBLICATION_RECEIPT_v0.62.14.json"
PREDECESSOR_GITHUB_RECEIPT = PROJECT / "GITHUB_PUBLICATION_RECEIPT_v0.62.14.json"
...
EXPECTED_PREDECESSOR_TOTAL_BYTES = 744_845_735
...
EXPECTED_PREDECESSOR_ZENODO_TOTAL_BYTES = 744_312_466
EXPECTED_PREDECESSOR_ZENODO_AGGREGATE = (
    "322938b537d631023c484017caf3235775760d8e6620036dd0f3a832d964a29d"
)
EXPECTED_PREDECESSOR_RECORD_ID = 22_217_240
EXPECTED_PREDECESSOR_TAG_COMMIT = "809baf41177fc4f0fca3c5f696c36be152ec2c01"
EXPECTED_PREDECESSOR_TAG_TREE = "d72037b889eb01acb7abc85151ccb5f989c77155"
EXPECTED_CONCEPT_ID = 22_059_707
EXPECTED_CONCEPT_DOI = "10.5281/zenodo.22059707"
EXPECTED_PREDECESSOR_DOI = "10.5281/zenodo.22217240"
EXPECTED_PREDECESSOR_STATE = "published_open_modular_backend_successor"
EXPECTED_PREDECESSOR_RELEASE_ID = 380_131_534

SOURCE_ZIP_NAME = "program-matematika-indonesia-source-v0.62.15.zip"
NOTES_NAME = "RELEASE_NOTES_v0.62.15.md"
CHECKSUM_NAME = "RELEASE_CHECKSUMS_v0.62.15.sha256"
...
EXPECTED_COURSE_CAPSULE_MEMBER_COUNT: int | None = 330
...
PREPARED_ADDITION_SOURCES: dict[str, Path] = {}
...
C130_ARCHIVE_PATH = PROJECT / "backend/course-capsule-v1/builds/program-matematika-indonesia-c130-operations-research-v2.3.1.zip"

V2_VALIDATED_SOURCE_PATHS = {
    "backend/course-capsule-v1/authority/v23-adapter-index-v2.json": REPLACEMENT_SOURCES[
        "v23-adapter-index-v2.json"
    ],
    "backend/course-capsule-v1/authority/modular-backend-pattern-index-v2.json": REPLACEMENT_SOURCES[
        "modular-backend-pattern-index-v2.json"
    ],
    "backend/course-capsule-v1/authority/feature-adoption-provenance-v1.json": REPLACEMENT_SOURCES[
        "feature-adoption-provenance-v1.json"
    ],
    "backend/course-capsule-v1/authority/comparison-evidence-manifest-v1.json": REPLACEMENT_SOURCES[
        "comparison-evidence-manifest-v1.json"
    ],
    "schemas/course-capsule-v1/v2/v23-adapter-index-v2.schema.json": REPLACEMENT_SOURCES[
        "v23-adapter-index-v2.schema.json"
    ],
    "schemas/course-capsule-v1/v2/modular-backend-pattern-index-v2.schema.json": REPLACEMENT_SOURCES[
        "modular-backend-pattern-index-v2.schema.json"
    ],
    "schemas/course-capsule-v1/v2/feature-adoption-provenance-v1.schema.json": REPLACEMENT_SOURCES[
        "feature-adoption-provenance-v1.schema.json"
    ],
    "schemas/course-capsule-v1/v2/comparison-evidence-manifest-v1.schema.json": REPLACEMENT_SOURCES[
        "comparison-evidence-manifest-v1.schema.json"
    ],
}
...
PREDECESSOR_ANONYMOUS_URL_ROOT = (
    f"https://github.com/KokunoYumeto/program-matematika-indonesia/releases/download/v{PREDECESSOR_VERSION}"
)
HYDRATION_PREFIX = ".v0.62.14-hydrate-v06215-"
BUILD_PREFIX = ".v0.62.15-build-"
...
def _validate_boundary(predecessor_names: set[str]) -> frozenset[str]:
    ...
    require(len(successor) == EXPECTED_FILES, "96+13+3 successor equation differs")
...
def _predecessor_inventory() -> tuple[dict[str, dict[str, Any]], frozenset[str]]:
    ...
    receipt = _strict_object(PREDECESSOR_RECEIPT, "v0.62.14 Zenodo publication receipt")
    ...
    require(receipt.get("payload_total_bytes") == EXPECTED_PREDECESSOR_ZENODO_TOTAL_BYTES, "predecessor Zenodo bytes differ")
    require(
        receipt.get("payload_inventory_aggregate_sha256") == EXPECTED_PREDECESSOR_ZENODO_AGGREGATE,
        "predecessor Zenodo aggregate differs",
    )
    ...
    require(
        ...
        and github.get("anonymous_readback") == "pass_112_of_112",
    ...
    require(
        ...
        and zenodo.get("file_count") == EXPECTED_PREDECESSOR_ZENODO_FILES
        and zenodo.get("anonymous_readback") == "pass_100_of_100",
        "predecessor Zenodo authority differs",
    )
    github_receipt = _strict_object(PREDECESSOR_GITHUB_RECEIPT, "v0.62.14 GitHub publication receipt")
    release = github_receipt.get("release")
    readback = github_receipt.get("anonymous_asset_readback")
    require(
        github_receipt.get("version") == PREDECESSOR_VERSION
        and github_receipt.get("tag") == f"v{PREDECESSOR_VERSION}"
        and isinstance(release, dict)
        and release.get("id") == EXPECTED_PREDECESSOR_RELEASE_ID
        and release.get("tag_target_commit") == EXPECTED_PREDECESSOR_TAG_COMMIT
        and release.get("tag_target_tree") == EXPECTED_PREDECESSOR_TAG_TREE
        and release.get("draft") is False
        and release.get("prerelease") is False,
        "predecessor GitHub release boundary differs",
    )
    require(
        ...
        "predecessor GitHub readback differs",
    )
    ...
    for raw in rows:
        ...
        expected_url = f"{PREDECESSOR_ANONYMOUS_URL_ROOT}/{quote(name, safe='')}"
        require(raw.get("url") == expected_url, f"predecessor anonymous URL differs: {name}")
...
def _validate_predecessor_directory(rows: dict[str, dict[str, Any]]) -> None:
    require(PREDECESSOR_DIR.is_dir() and not PREDECESSOR_DIR.is_symlink(), "local v0.62.14 payload is missing")
...
def _validate_c130(source_commit: str) -> dict[str, Any]:
    ...
    # This sealed ADMISSION row is historical pre-v0.62.14 evidence.  Current
    # publication state is asserted from the v2 snapshot below.
    require(admission.get("state") == "locally_admitted_central_release_pending", "historical C130 admission state differs")
    ...
    archive_path = C130_ARCHIVE_PATH
    require(_identity(archive_path) == EXPECTED_C130_ARCHIVE, "C130 package identity differs")
    require_committed_bytes(source_commit, archive_path, archive_path.read_bytes(), "C130 package")
...
def _validate_v2_snapshot(source_commit: str) -> dict[str, Any]:
    ...
    index = _strict_object(REPLACEMENT_SOURCES["v23-adapter-index-v2.json"], "v2 adapter index")
    ...
    require(c130.get("admission_state") == "published", "C130 v2 admission state differs")
    require(
        c130.get("public_replay_status") == "published_public_asset_readback_verified"
        and isinstance(c130.get("release_url"), str)
        and isinstance(c130.get("public_asset_url"), str)
        and "planned_release" not in c130,
        "C130 v2 public replay boundary differs",
    )
    require(c130.get("archive", {}).get("sha256") == EXPECTED_C130_ARCHIVE[1], "C130 v2 archive binding differs")

    receipt_path = REPLACEMENT_SOURCES["MODULAR_BACKEND_SNAPSHOT_V2_RECEIPT.json"]
...
def _validate_capsule_archive(path: Path, flat_jsonl: bytes) -> dict[str, Any]:
    ...
    require(
        (len(capsule_data), sha256(capsule_data)) == EXPECTED_COURSE_CAPSULE_IDENTITY,
        "combined capsule byte identity differs",
    )
    validate_zip(
        COURSE_CAPSULE_ARCHIVE_NAME,
        path,
    ...
    return {
        "bytes": EXPECTED_COURSE_CAPSULE_IDENTITY[0],
        "sha256": EXPECTED_COURSE_CAPSULE_IDENTITY[1],
        "members": EXPECTED_COURSE_CAPSULE_MEMBER_COUNT,
        "c130_package_bound": True,
        "historical_c130_admission_preserved": True,
    }
...
def _collect_prepared(source_commit: str) -> tuple[list[dict[str, Any]], dict[str, bytes], dict[str, Any]]:
    ...
    for name, path in sorted(sources.items()):
        ...
        rows.append(fact(name, data, "commit_bound_v0.62.15_source", path))
...
def _notes(source: dict[str, Any]) -> bytes:
    text = f"""# Program Matematika Indonesia v0.62.15

Rilis ini mempertahankan 96 berkas v0.62.14 secara byte-identik, mengganti
tiga belas nama, menghilangkan hanya tiga berkas versi lama, dan menambah tiga
...
Semua delapan paket adapter sudah terbit dan dibaca balik; 31 peran tetap
tanpa klaim adapter lokal.

Permukaan backend Bahasa Indonesia kini memiliki fallback tanpa JavaScript
yang benar-benar berorientasi pelajar, rute PDF D90 yang jujur, lima alat
belajar terverifikasi, dan label akses yang dapat dibaca pembaca layar.
...
def _assemble(destination: Path, source_commit: str, source_tree: str) -> list[dict[str, Any]]:
    ...
    for name in sorted(retained):
        ...
        rows.append(fact(name, data, "retained_exact_from_v0.62.14"))
    ...
    require(len(rows) == EXPECTED_CHECKSUM_ROWS, "96+13+1+1 pre-checksum equation differs")
...
def _compare_existing(expected_dir: Path, rows: list[dict[str, Any]]) -> None:
    require(OUTPUT_DIR.is_dir() and not OUTPUT_DIR.is_symlink(), "existing v0.62.15 output is not a regular directory")
    entries = list(OUTPUT_DIR.iterdir())
    require(len(entries) == EXPECTED_FILES, "existing v0.62.15 count differs")
    require(all(path.is_file() and not path.is_symlink() for path in entries), "existing v0.62.15 is not flat regular files")
    names = {str(row["name"]) for row in rows}
    require({path.name for path in entries} == names, "existing v0.62.15 names differ")
    for name in sorted(names):
        require((OUTPUT_DIR / name).read_bytes() == (expected_dir / name).read_bytes(), f"existing v0.62.15 bytes differ: {name}")


def _run_preflight(source_commit: str, source_tree: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="pmi-v0.62.15-preflight-") as temporary:
...
def _run_hydrate() -> dict[str, Any]:
    ...
    try:
        ...
        for name in sorted(rows):
            ...
            request = Request(
                str(row["url"]),
                headers={"Accept": "*/*", "Accept-Encoding": "identity", "User-Agent": "PMI-v06215-predecessor-hydrator/1"},
                method="GET",
            )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    modes.add_argument("--build", action="store_true", help="atomically assemble releases/v0.62.15")
    modes.add_argument("--hydrate-predecessor", action="store_true", help="anonymously restore an absent pinned v0.62.14 GitHub payload")
