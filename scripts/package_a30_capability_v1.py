"""Create the deterministic metadata-only A30 capability packet."""
...
from validate_a30_capability_v1 import validate


ADAPTER_REL = Path("backend/course-capsule-v1/adapters/a30-capability-v1")
DEFAULT_OUTPUT = DEFAULT_ADAPTER / "build"
ARCHIVE_NAME = "A30_THIN_CAPABILITY_METADATA_V1.zip"
RECEIPT_NAME = "PACKET_BUILD_RECEIPT.json"
SCRIPT_NAMES = (
    "a30_capability_model_v1.py",
    "build_a30_capability_v1.py",
    "validate_a30_capability_v1.py",
    "package_a30_capability_v1.py",
    "verify_a30_native_public_v1.py",
)
FORBIDDEN_SUFFIXES = {
...
def safe_relative_path(value: str) -> PurePosixPath:
    ...
    if not value or path.is_absolute() or any(part in {"", ".", ".."} for part in path.parts) or (path.parts and ":" in path.parts[0]):
        raise PacketError("A30-PACKET-UNSAFE-PATH:" + value)
    return path
...
def _add_member(members: dict[str, bytes], name: str, path: Path) -> None:
    ...
    if not safe_member(normalized) or normalized in members:
        raise PacketError("A30-PACKET-MEMBER-PATH:" + normalized)
    if path.suffix.casefold() in FORBIDDEN_SUFFIXES:
        raise PacketError("A30-PACKET-FORBIDDEN-PAYLOAD:" + normalized)
    data = path.read_bytes()
    if LOCAL_PROFILE_PATTERN.search(data):
        raise PacketError("A30-PACKET-LOCAL-PROFILE-DATA:" + normalized)
...
def collect_members(adapter: Path) -> dict[str, bytes]:
    ...
    for path in required:
        if not path.is_file():
            raise PacketError("A30-PACKET-REQUIRED-FILE-MISSING:" + path.name)
    ...
    if actual_adapter != allowed_adapter:
        ...
        raise PacketError(f"A30-PACKET-ADAPTER-FILE-SET:missing={missing}:extra={extra}")

    expected_tooling = [f"scripts/{name}" for name in SCRIPT_NAMES]
    if manifest.get("tooling_scripts") != expected_tooling:
        raise PacketError("A30-PACKET-TOOLING-MANIFEST")
    ...
    members["START_HERE.md"] = (
        ...
        "This packet contains only A30 adapter metadata, stable-identity indexes, generated "
        "learner and educator views, validation evidence, negative fixtures, and Python standard-"
        "library scripts. It does not contain the native backend dataset, source text, target text, "
        "textbook bodies, segment text, exercise/problem/solution bodies, TeX, PDFs, EPUBs, images, "
        "credentials, caches, or logs.\n\n"
        "Deterministic replay requires the separately preserved final public backend-core ZIP, the "
        "pinned producer QA evidence listed in `input/source-lock.json`, and the source companion "
        "required by the producer for full replay. The canonical JSONL member SHA-256 is "
        f"`{source_lock['native_export']['jsonl_sha256']}`. Then run:\n\n"
        ...
        "A20 is a central curriculum overlay only. The packet preserves the exact 4,183/3,067 "
        "solution-supported/unsupported boundary and segment-state asymmetry. It does not claim a "
        "final derivative Git commit/tree, semantic HTML, MathML, EPUB, portable HTML, PDF/UA, WCAG "
        "conformance, labs, a runtime, an official teacher manual, per-exercise PDF destinations, "
        "standalone raw replay, or reversible exchange.\n"
    ).encode("utf-8")
    inventory = {
        "schema": "a30-capability-thin-packet-inventory/1",
        "course_id": COURSE_ID,
        "content_policy": "adapter_metadata_only_external_public_native_replay_dependency",
        ...
        "source_or_target_text_included": False,
        "exercise_or_solution_bodies_included": False,
        "exercise_pdf_destinations_included": False,
        ...
        "excluded_payload_classes": [
            ...
            "source text, target text, and textbook or segment bodies",
            "exercise, problem, and solution bodies",
...
def verify_zip(payload: bytes, members: dict[str, bytes]) -> dict[str, Any]:
    with zipfile.ZipFile(io.BytesIO(payload), "r") as archive:
        ...
        if names != sorted(members) or len(names) != len(set(names)):
            raise PacketError("A30-PACKET-ZIP-INVENTORY")
        if archive.testzip() is not None:
            raise PacketError("A30-PACKET-ZIP-CRC")
        for info in infos:
            if not safe_member(info.filename):
                raise PacketError("A30-PACKET-ZIP-PATH:" + info.filename)
            if info.date_time != (1980, 1, 1, 0, 0, 0) or info.extra or info.comment:
                raise PacketError("A30-PACKET-ZIP-METADATA:" + info.filename)
            data = archive.read(info.filename)
            if data != members[info.filename] or info.file_size != len(data):
                raise PacketError("A30-PACKET-ZIP-BYTES:" + info.filename)
            if PurePosixPath(info.filename).suffix.casefold() in FORBIDDEN_SUFFIXES:
                raise PacketError("A30-PACKET-ZIP-FORBIDDEN-PAYLOAD:" + info.filename)
            if LOCAL_PROFILE_PATTERN.search(data):
                raise PacketError("A30-PACKET-ZIP-LOCAL-PROFILE:" + info.filename)
    return {
...
def package(native: Path, adapter: Path, output: Path, receipt_path: Path = PUBLIC_RECEIPT_PATH) -> dict[str, Any]:
    ...
    if first != second:
        raise PacketError("A30-PACKET-TWO-BUILD-DRIFT")
    ...
    receipt = {
        "schema": "a30-capability-thin-packet-build-receipt/1",
        "course_id": COURSE_ID,
        "result": "PASS",
...
def main() -> int:
    ...
    receipt = package(args.native_root.resolve(), args.adapter.resolve(), args.output.resolve(), args.receipt.resolve())
...
if __name__ == "__main__":
    try:
        ...
        print(f"A30 packet build failed: {error}", file=sys.stderr)
        sys.exit(1)
