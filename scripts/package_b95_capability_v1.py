"""Create a deterministic, metadata-only B95 capability packet."""
...
from validate_b95_capability_v1 import validate


ADAPTER_REL = Path("backend/course-capsule-v1/adapters/b95-capability-v1")
DEFAULT_OUTPUT = DEFAULT_ADAPTER / "build"
ARCHIVE_NAME = "B95_THIN_CAPABILITY_METADATA_V1.zip"
RECEIPT_NAME = "PACKET_BUILD_RECEIPT.json"
SCRIPT_NAMES = (
    "b95_capability_model_v1.py",
    "build_b95_capability_v1.py",
    "validate_b95_capability_v1.py",
    "package_b95_capability_v1.py",
    "verify_b95_native_public_v1.py",
)
FORBIDDEN_SUFFIXES = {
...
def safe_relative_path(value: str) -> PurePosixPath:
    ...
    if (
        ...
        raise PacketError("B95-PACKET-UNSAFE-PATH:" + value)
    return path
...
def _add_member(members: dict[str, bytes], name: str, path: Path) -> None:
    ...
    if not safe_member(normalized) or normalized in members:
        raise PacketError("B95-PACKET-MEMBER-PATH:" + normalized)
    if path.suffix.casefold() in FORBIDDEN_SUFFIXES:
        raise PacketError("B95-PACKET-FORBIDDEN-PAYLOAD:" + normalized)
    data = path.read_bytes()
    if LOCAL_PROFILE_PATTERN.search(data):
        raise PacketError("B95-PACKET-LOCAL-PROFILE-DATA:" + normalized)
...
def collect_members(adapter: Path) -> dict[str, bytes]:
    ...
    for path in required:
        if not path.is_file():
            raise PacketError("B95-PACKET-REQUIRED-FILE-MISSING:" + path.name)
    ...
    if actual_adapter != allowed_adapter:
        ...
        raise PacketError(f"B95-PACKET-ADAPTER-FILE-SET:missing={missing}:extra={extra}")
    ...
    members["START_HERE.md"] = (
        ...
        "This packet contains only B95/R011-B039 metadata, stable identity indexes, generated "
        "learner and educator views, validation evidence, negative fixtures, and standard-library "
        "Python scripts. It does not contain the native backend dataset, textbook or exercise "
        "bodies, answers, solutions, TeX, PDFs, credentials, caches, or logs.\n\n"
        "Deterministic replay requires the separately preserved native checkout and the exact "
        "hash-pinned source inputs recorded in the adapter's input/source-lock.json. Then run:\n\n"
        ...
        "The adapter preserves native identifiers, structure, terminology, rights, and public "
        "release evidence while making no semantic-HTML, WCAG, EPUB, offline, answer-key, or "
        "reversible-exchange claim.\n"
    ).encode("utf-8")
    inventory = {
        "schema": "b95-capability-thin-packet-inventory/1",
        "course_id": COURSE_ID,
        ...
        "excluded_payload_classes": [
            ...
            "textbook, segment, exercise, problem, answer, and solution bodies",
...
def verify_zip(payload: bytes, members: dict[str, bytes]) -> dict[str, Any]:
    with zipfile.ZipFile(io.BytesIO(payload), "r") as archive:
        ...
        if names != sorted(members) or len(names) != len(set(names)):
            raise PacketError("B95-PACKET-ZIP-INVENTORY")
        if archive.testzip() is not None:
            raise PacketError("B95-PACKET-ZIP-CRC")
        for info in infos:
            if not safe_member(info.filename):
                raise PacketError("B95-PACKET-ZIP-PATH:" + info.filename)
            if info.date_time != (1980, 1, 1, 0, 0, 0) or info.extra or info.comment:
                raise PacketError("B95-PACKET-ZIP-METADATA:" + info.filename)
            data = archive.read(info.filename)
            if data != members[info.filename] or info.file_size != len(data):
                raise PacketError("B95-PACKET-ZIP-BYTES:" + info.filename)
            if PurePosixPath(info.filename).suffix.casefold() in FORBIDDEN_SUFFIXES:
                raise PacketError("B95-PACKET-ZIP-FORBIDDEN-PAYLOAD:" + info.filename)
            if LOCAL_PROFILE_PATTERN.search(data):
                raise PacketError("B95-PACKET-ZIP-LOCAL-PROFILE:" + info.filename)
    return {
...
def package(native: Path, adapter: Path, output: Path) -> dict[str, Any]:
    ...
    if first != second:
        raise PacketError("B95-PACKET-TWO-BUILD-DRIFT")
    zip_checks = verify_zip(first, members)
    output.mkdir(parents=True, exist_ok=True)
    archive_path = output / ARCHIVE_NAME
    write_bytes(archive_path, first)
    receipt = {
        "schema": "b95-capability-thin-packet-build-receipt/1",
        "course_id": COURSE_ID,
        "result": "PASS",
...
if __name__ == "__main__":
    try:
        ...
        print(f"B95 packet build failed: {error}", file=sys.stderr)
        sys.exit(1)
