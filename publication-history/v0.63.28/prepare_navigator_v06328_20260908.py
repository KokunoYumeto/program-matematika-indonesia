...
COMMIT='818dafebc973a6eff1330fcf6be86db8400192fd'
BASE='a593c3b32eea860c3f1a7d7009093cc4ca2c17d4'
PREDECESSOR_SHA='b4e41abaa81b2c23e6274b666a58547557cff6495b96dc851ce3afe976a437d5'
PACKAGE_SHA='ad4c91eb6af25d38ef835df9ffd0654346558ea75dff71e6299015f96de0d11f'
CHECKSUM='CHECKSUMS.sha256'
PROJECTION='PORTABLE_PROJECTION.json'
PACKAGE_NAME='original-sources/A30-Precalculus-2e-original-English-reader-source-v1.zip'
...
def run(repo,old,package,out,helper):
    ...
    assert git('rev-parse',COMMIT+'^{tree}').decode().strip()=='1d31d37f9d51226490782261c792f1820318580b'
    ...
    with zipfile.ZipFile(old) as previous:
        old_names=previous.namelist();assert len(old_names)==6231==len(set(old_names))
            ...
            with zipfile.ZipFile(out,'x',compression=zipfile.ZIP_DEFLATED,compresslevel=6,allowZip64=True) as target:
                ...
                for number,name in enumerate(names,1):
                    ...
                    if name=='SOURCE_COMMIT.txt':data=(COMMIT+'\n').encode()
                        ...
                        data=base.START.replace(b'</ul>',b'<li><a href="docs/en/courses/A30/reader/index.html">A30: Precalculus 2e - all 87 original-English modules</a></li><li><a href="'+PACKAGE_NAME.encode()+b'">Precalculus standalone reader and original-source package</a></li></ul>',1)
                        data=data.replace(b'and A10 reader open',b'and the A10/A30 readers open')
                    ...
                    if name.startswith('docs/') and name.endswith('.html'):
                        ...
                        insertion=('\n<script src="'+rel+'" data-portable-navigation="v0.63.28"></script>\n').encode()
                        at=data.lower().rfind(b'</body>');data=data[:at]+insertion+data[at:] if at>=0 else data+insertion
                        assert data.count(b'data-portable-navigation="v0.63.28"')==1
    ...
    with zipfile.ZipFile(out) as z:
        ...
        assert len([n for n in rows if n.startswith('docs/en/courses/A30/reader/modules/') and n.endswith('.html')])==87
    ...
    out.with_name('NAVIGATOR_V06328_LOCAL_RECEIPT.json').write_bytes(jb(result));print(json.dumps(result),flush=True)
if __name__=='__main__':
    p=argparse.ArgumentParser()
    for flag in ('repo','predecessor','a30-package','output','v27-helper'):p.add_argument('--'+flag,required=True,type=Path)
    a=p.parse_args();run(a.repo.resolve(),a.predecessor.resolve(),a.a30_package.resolve(),a.output.resolve(),a.v27_helper.resolve())
