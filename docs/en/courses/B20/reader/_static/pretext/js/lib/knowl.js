  ...
  async getContent() {
    ...
    const knowlContent = await fetch(contentURL)
      ...
      .then((data) => {
        ...
        // B20 standalone excerpts retain reading runtime and program navigation.
        // Embedded excerpts use the parent's runtime and never duplicate that chrome.
        knowlDoc.querySelectorAll('[data-b20-standalone-only], [data-central-surface-navigation="v1"]').forEach(n => n.remove());
    ...
    if (this.outputElement !== null) {
      ...
      content
        .then((tempContainer) => {
          ...
          if (window.Prism) window.Prism.highlightAllUnder(this.outputElement);
