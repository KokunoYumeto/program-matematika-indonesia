  ...
  async getContent() {
    ...
    const knowlContent = await fetch(contentURL)
      ...
      .then((data) => {
        ...
        // B10 hosted fragment navigation: keep the program shell on standalone
        // excerpts, but not inside an excerpt expanded within a reader page.
        tempContainer.querySelectorAll('[data-central-surface-navigation="v1"]').forEach(n => n.remove());
        // Standalone excerpt URLs are document-relative. Resolve them before
        // transplanting the body into the chapter's different URL base.
        const excerptBase = new URL(contentURL, document.baseURI);
        for (const el of tempContainer.querySelectorAll('[href],[src],[data-knowl],[data-src],[poster]')) {
          for (const attr of ['href','src','data-knowl','data-src','poster']) {
            const value = el.getAttribute(attr);
            if (value && !value.startsWith('#') && !/^(?:[a-z][a-z0-9+.-]*:|\/\/)/i.test(value)) {
              el.setAttribute(attr, new URL(value, excerptBase).href);
            }
          }
        }
