...
A00 is now the first verified original-English mirror under the language-independent access policy. Its program-hosted reader preserves all 75 OpenStax modules, 2,958 original assets, 22,221 MathML expressions, 8,105 exercises, 5,240 solutions, 48,319 native IDs, original credits, and CC BY-NC-SA 4.0 component distinctions. The OpenStax publisher page remains a separate prominent action. The self-contained HTML ZIP works after extraction without JavaScript or remote rendering; external citations and OpenStax services still require internet. This is a presentation mirror of frozen original English source, not a translation or evidence that the remaining roles are mirrored.
